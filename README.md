...
![Version](https://img.shields.io/badge/version-0.0.73--dev-00BFA5)
...
- терпимый импорт сторонних feeds: корректные профили сохраняются даже при отдельных ошибочных поддерживаемых записях, а ошибки учитываются отдельно;
...
- задать отображаемое имя подписки;
...
- проверка задержки для протоколов, где probe не требует захвата Android VPN slot;
...
Выбранные приложения поднимаются вверх списка, чтобы их не приходилось повторно искать среди всех установленных.
...
IPv6 отображается только когда его наличие реально подтверждено. Если IPv6 нет, интерфейс не показывает фиктивные `?`, `Unknown` или `N/A`.
...
Простое открытие приложения **не забирает Android VPN slot** у уже работающего VPN. В актуальной development-линии диагностические latency/probe действия также не должны создавать временный Android VPN: системный VPN slot предназначен для настоящего Connect/Reconnect, а не для фоновой диагностики.
...
- latency/probe там, где тест можно выполнить без захвата системного VPN;
- состояние доступности;
- журнал событий;
- диагностика активного профиля;
- отдельные сообщения об ошибках импорта и runtime.

В актуальной development-линии окно диагностики не должно самопроизвольно закрываться от внутренних действий. Оно остаётся открытым до явного Back / Close / dismiss пользователя.

Для **AWG2/AWG3** диагностический latency через временный Android VPN отключён: реальное подключение имеет абсолютный приоритет над диагностикой.
...
> **SG Mobile 0.0.73 / versionCode 173 — P73 Connect Stability development candidate.**

Последняя серия P70 → P73 включает:

- полную VLESS parity-обработку используемых RAW/TCP, WebSocket, gRPC, HTTPUpgrade и XHTTP вариантов с REALITY/TLS/`security=none`;
- устойчивый импорт сторонних подписок без потери корректных профилей из-за отдельных ошибочных строк;
- строгий SG Subscription v1 contract;
- стабильное окно диагностики, которое не должно закрываться от внутренних действий;
- чистое отображение IPv6 только при реально подтверждённом IPv6;
- удаление временного AWG Android-VPN latency probe из диагностического пути;
- приоритет настоящего Connect над любыми диагностическими проверками.

Подробности: [P73 development release notes](docs/releases/0.0.73-p73.md) и [CHANGELOG](CHANGELOG.md).

> P73 не объявляется стабильным бинарным релизом до повторного полного Windows Gradle/R8 build gate после последней синхронизации regression-тестов.
...
Development candidate считается готовым к бинарной публикации только после полного Windows regression/unit-test gate и успешного Release APK build.
